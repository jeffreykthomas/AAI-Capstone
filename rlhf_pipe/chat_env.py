...
class ChatEnvironment(gym.Env):
	...
	def __init__(self, model, tokenizer, max_seq_len, samples, reward_function):
		...
		self.current_conversation_lenth = 0
	...
	def _next_observation(self):
		...
		self.current_conversation_lenth += 1
		output = self.tokenizer(self.current_state, return_tensors="pt")
	...
	def step(self, action):
		...
		with torch.no_grad():
			response1 = self.model.generate(self.current_state, 256)
			response2 = self.model.generate(self.current_state, 256)

		# Calculate reward based on the generated response
		reward = self.reward_function(self.current_state, response1, response2)
		...
		done = self.current_conversation_lenth >= 10
