...
# Based on LlaMA: https://arxiv.org/abs/2302.13971v1
...
def adjust_for_broadcast(rotary_frequencies, inputs):
...
def apply_rotary_embeddings(query, key, frequencies):
...
class MultiHeadAttention(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.key_cache = torch.zeros(config.max_batch_size, config.max_sequence_length, self.num_heads, self.dimension_per_head).cuda()
        self.value_cache = torch.zeros(config.max_batch_size, config.max_sequence_length, self.num_heads, self.dimension_per_head).cuda()
    ...
    def forward(self, inputs, start_position, rotary_frequencies, attention_mask: Optional[torch.Tensor] = None):
        ...
        query, key, value = [x.view(batch_size, sequence_length, self.num_heads, self.dimension_per_head) for x in [query, key, value]]
...
class FeedForwardNetwork(nn.Module):
    def __init__(self, dimension: int, hidden_dimension: int, rounding_multiple: int):
        ...
        adjusted_hidden_dimension = rounding_multiple * ((adjusted_hidden_dimension + rounding_multiple - 1) // rounding_multiple)
...
class TransformerLayer(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.feed_forward = FeedForwardNetwork(config.model_dimension, config.model_dimension * config.swiglu_multiple, 256)
...
class TransformerModel(nn.Module):
    def __init__(self, config: TransformerConfig):
        ...
        self.positional_embedding = compute_rotary_embedding_frequencies(config.model_dimension, config.max_sequence_length)
